...
export default async function EvidenceIndex({params}:{params:Promise<{locale:string}>}){
 ...
 <h2>Coded institutional attributes</h2>{data['institution-attributes'].map(a=><section className={styles.record} key={a.id}><h3>{data.institutions.find(i=>i.id===a.institutionId)?.label} · {data['attribute-rubrics'].find(r=>r.id===a.rubricId)?.label}</h3><p>{a.value} · {a.assessedOn}</p><p>{a.reasoning}</p><p>{a.limitations}</p><a href={`${prefix}/institutional-links/${data.relationships.find(r=>[r.source,r.target].includes(a.institutionId))?.id}#${a.id}`}>Inspect evidence and rubric →</a></section>)}
 ...
 <p><a href={`https://github.com/saykig/Cepheus/blob/${data.release.methodologyCommit}/research/METHODOLOGY.md`}>Release-pinned methodology →</a></p><p><a href="/data/institutional-map/bundle.json">Download the publication bundle</a></p>
